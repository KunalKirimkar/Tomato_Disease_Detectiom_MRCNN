...
COCO_MODEL_PATH = r'D:/Tomato_Disease_Detection_MRCNN/mask_rcnn_coco.h5'

K.set_learning_phase(0)
FLAGS = flags.FLAGS

flags.DEFINE_string('input_model', default=r'D:/Tomato_Disease_Detection_MRCNN/logs/object20231005T1159/mask_rcnn_object_0010.h5', help='Path to the input model.')
...
flags.DEFINE_string('output_model', default=r'./shapes20191113T1540_mask_rcnn_shapes_0010.pb', help='Path where the converted model will be stored.')
...
def load_model(input_model_path, input_json_path=None, input_yaml_path=None):
    ...
    try:
        model = modellib.MaskRCNN(mode="training", config=config,
                                  model_dir='D:/Tomato_Disease_Detection_MRCNN/logs/object20231005T1159/')
        model = keras.models.load_model('D:/Tomato_Disease_Detection_MRCNN/logs/object20231005T1159/mask_rcnn_object_0010.h5')
        return model
...
class ShapesConfig(Config):
    ...
    NUM_CLASSES = 1 + 10  # background + 15 object
...
def main(args):
    ...
    config = ShapesConfig()
    config.display()
    MODEL_DIR = r'D:/Tomato_Disease_Detection_MRCNN/logs'
    ...
    # orig_output_node_names = [node.op.name for node in model.keras_model.outputs]
    ...
    if FLAGS.output_nodes_prefix:
        ...
        for i in range(num_output):
            ...
            pred[i] = tf.identity(model.keras_model.outputs[i],
    ...
    if FLAGS.save_graph_def:
        tf.compat.v1.train.write_graph(sess.graph.as_graph_def(), str(output_fld),
